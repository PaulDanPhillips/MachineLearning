...
class Simulation_Analysis:
    def __init__(self, cntrl_path, pyseer_path, boruta_path, regress_path, predictor_path, response_path):
    ...
    def control_gene_lst(self, cntrl_path):
        ...
        for filename in all_files:
            ...
            df1['Simulation'] = df1['Simulation'].replace("_phenSim\.par", '', regex=True)
        ...
        return cntrl_lst_DF
    def predictor_list(self, predictor_path):
        ...
        for filename in all_files:
            ...
            # df1.set_index('QTL', inplace=True)
            predictor_lst_df.append(df1)
        # order = [0, 2, 3, 4, 5, 6, 7, 8, 9, 1] # Needed to be re-ordered as the gene list by simulation number
        # predictor_lst_df = [predictor_lst_df[i] for i in order]    
        predDF =pd.concat(predictor_lst_df).sort_values('Simulation')
        grouped_cntrl = predDF.groupby('Simulation')
        predictor_lst_df = []
        for i in grouped_cntrl:
            predictor_lst_df.append(i[1])
        predictor_lst_df2 = []
        for i in predictor_lst_df:
            i = i.dropna(axis=1)
            predictor_lst_df2.append(i)
        return predictor_lst_df2
    
    def response_list(self, response_path):
        os.chdir(response_path)
        all_files=sorted(glob.glob("*.phen"))# add sorted around all glob.glob
        response_lst_df = []

        for filename in all_files:
            df1 = pd.read_csv(filename, sep=' ', header=None, index_col=0).drop(columns=[1,3])
            df1.columns=['RESPONSE']
            df1['Simulation'] = str(filename)
            df1['Simulation'] = df1['Simulation'].replace("_phenSim\.phen", '', regex=True)
            # df1.set_index('QTL', inplace=True)
            response_lst_df.append(df1)
        # order = [0, 2, 3, 4, 5, 6, 7, 8, 9, 1] # Needed to be re-ordered as the gene list by simulation number
        # response_lst_df = [response_lst_df[i] for i in order]    
        respDF =pd.concat(response_lst_df).sort_values('Simulation')
        grouped_cntrl = respDF.groupby('Simulation')
        response_lst_df = []
        for i in grouped_cntrl:
            response_lst_df.append(i[1])
        return response_lst_df
